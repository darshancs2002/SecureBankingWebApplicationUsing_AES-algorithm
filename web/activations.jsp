<%-- 
    Document   : test1
    Created on : 22 Jun, 2023, 9:11:26 AM
    Author     : Darshan CS
--%>
...
<html>
    ...
    <body>
        ...
        <div class="outer">
            <table>
                <th id="tab">Activations</th>
                <tr><th>FIRST NAME</th><th>LAST NAME</th><th>EMAIL</th><th>MOBILE</th><th>STATE</th><th>STATUS</th><th>ACTION</th></tr>

                <%
                    rst = stmt.executeQuery("select * from activeuserss where isActive='False'");
                    while (rst.next()) {
                        out.println("<tr><td>" + rst.getString("first") + "</td><td>" + rst.getString("second") + "</td><td>" + rst.getString("email") + "</td><td>" + rst.getString("phone") + "</td><td>" + rst.getString("state") + "</td>");
                %><td style="color:green ; font-weight: bold;">waiting</td> <td><a href="enter.jsp?email=<%=rst.getString("email")%>">View</a></td>
                <%
                    }
                %>
            </table>
